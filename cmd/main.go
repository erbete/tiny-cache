...
func main() {
	...
	go func() {
		if err := tinycache.Run(addr); err != nil {
			log.Fatal(err)
		}
	}()

	// for i := 0; i < 10; i++ {
	// 	err := clientMain(addr, "hi from client")
	// 	if err != nil {
	// 		log.Fatal(err)
	// 	}
	// }
...
// TODO: remove this; quickly used for testing
...
func clientMain(address, message string) error {
	tlsConf := &tls.Config{
	...
	response, err := client.Add(ctx, &proto.AddRequest{
		Key:     "1",
		Data:    "Test",
		Expires: "10m",
	})
	...
	// dialer, err := quic.DialAddr(address, tlsConf, nil)
	// if err != nil {
	// 	return err
	// }
